    ...
    if (
      ...
      if (values.empleado > 20) {
        if (Sucursal.length === 0) {
          ...
          for (let x = 0; x <= restantes; x++) {
        if(x<=Sucursal.length){
              if (Sucursal[x].empleado == "20") {
                return;
              } 
              else {
                let emp;
                emp = parseInt(Sucursal[x].empleado) + 1;
                const id = Sucursal[x].id;
                const obj = {
            ...
            restantes=restantes-Sucursal.length
            x=0;
